...
for iter=cont:NF  
 ...
 %%   delficiones 
        ...
        F=2*Afi*((3*fi.^2-1-2*ep1*fi.*(bet*u.^2)).*mu-2*ep*lapmu);     
...
%%        
        ...
        Gup = As*u+2*Ab*(fi.^2-1).*(u-u1).*(u-u2).*(2*u-u1-u2)+2*Af*fi.^2.*(u-u3);
       Gu=-4*Afi*ep1*bet*u.*mu.*((fi).^2-1)-sifiu*lapfi+Gup; 
      ...
      gFu=grad3DR(Fu);
        ...
        F1 = 4*Ab*fi.*(fi.^2-1).*(u-u1).^2.*(u-u2).^2 +2*Af*fi.*(u-u3).^2;
        H = F1;
        lap3Dt;
        lapF1 = lapH;
...
%%   tensor de esfuerzos
        ...
        gmu=grad3DR(mu);
        gu=grad3DR(u);
        P=(Afi*mu.^2-sigma*abs(gfi(:,:,:,1).^2+gfi(:,:,:,2).^2+gfi(:,:,:,3).^2)+0.5*Av*duu*abs(gu(:,:,:,1).^2+gu(:,:,:,2).^2+gu(:,:,:,3).^2)...
           +sifiu*(gfi(:,:,:,1).*gu(:,:,:,1)+gfi(:,:,:,2).*gu(:,:,:,2)+gfi(:,:,:,3).*gu(:,:,:,3))+0.5*As*u.^2 +Ab*(fi.^2-1).^2.*(u-u1).^2.*(u-u2).^2 ...
